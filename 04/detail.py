...
def fetch_page_source(
    ...
    try:
        ...
        logger.error(f"[ERROR] ページの取得に失敗: {url} - {e}")
        return None

    try:
        soup = BeautifulSoup(response.text, "html.parser")
    except Exception as e:
        logger.error(f"[ERROR] HTMLパースに失敗: {url} - {e}")
        return None
    ...
    try:
        ...
        if body_text:
            panel = body_text.select_one(".panel.panel-default")
            if panel:
                panel.decompose()
            body_text = body_text.get_text()
        else:
            body_text = ""
            logger.error(f"[ERROR] body_textの取得に失敗: {url}")
    except Exception as e:
        logger.error(f"[ERROR] body_textの処理中にエラー発生: {url} - {e}")
        return None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--file", type=str, required=True, help="CSV file containing job links"
    )
    parser.add_argument(
    ...
    output_file = "jobs_detail.csv"
    pd.DataFrame([job.to_json() for job in job_details]).to_csv(
