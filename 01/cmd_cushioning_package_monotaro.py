...
from monotaro_2024_12.monotaro import CUSHIONING_PACKAGE as MCUSHIONING_PACKAGE
...
try:
    monotaro_cushioning_package_response = requests.get(
        MCUSHIONING_PACKAGE["url"],
        headers=HEADERS,
    )
    monotaro_cushioning_package_response.encoding = monotaro_cushioning_package_response.apparent_encoding
    monotaro_cushioning_package_response.raise_for_status()  # ステータスコードがエラーの場合例外を発生
except requests.exceptions.RequestException as e:
    print("error occured on requests.get(ranking_url, headers=HEADERS): ", e)

ranking_page = MRankingPage(html=monotaro_cushioning_package_response.text)
...
# CSVファイルに保存
csv_file = "cushioning_package_products(モノタロウ).csv"
