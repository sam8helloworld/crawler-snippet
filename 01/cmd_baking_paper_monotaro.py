...
from monotaro_2024_12.monotaro import BAKING_PAPER as MBAKING_PAPER
...
try:
    monotaro_baking_paper_response = requests.get(
        MBAKING_PAPER["url"],
        headers=HEADERS,
    )
    monotaro_baking_paper_response.encoding = monotaro_baking_paper_response.apparent_encoding
    monotaro_baking_paper_response.raise_for_status()  # ステータスコードがエラーの場合例外を発生
except requests.exceptions.RequestException as e:
    print("error occured on requests.get(ranking_url, headers=HEADERS): ", e)

ranking_page = MRankingPage(html=monotaro_baking_paper_response.text)
...
# CSVファイルに保存
csv_file = "baking_paper_products(モノタロウ).csv"
