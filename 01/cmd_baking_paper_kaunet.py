...
from kaunet_2024_12.kaunet import BAKING_PAPER as KBAKING_PAPER
...
try:
    kaunet_baking_paper_response = requests.get(
        KBAKING_PAPER["url"],
        headers=HEADERS,
    )
    kaunet_baking_paper_response.encoding = kaunet_baking_paper_response.apparent_encoding
    kaunet_baking_paper_response.raise_for_status()  # ステータスコードがエラーの場合例外を発生
except requests.exceptions.RequestException as e:
    print("error occured on requests.get(ranking_url, headers=HEADERS): ", e)

ranking_page = KRankingPage(html=kaunet_baking_paper_response.text)
...
# CSVファイルに保存
csv_file = "baking_paper_products(カウネット).csv"
