...
from tanomail_2024_12.tanomail import CUSHIONING_PACKAGE as TCUSHIONING_PACKAGE
...
try:
    tanomail_cashioning_package__response = requests.get(
        TCUSHIONING_PACKAGE["url"],
        headers=HEADERS,
    )
    tanomail_cashioning_package__response.encoding = tanomail_cashioning_package__response.apparent_encoding
    tanomail_cashioning_package__response.raise_for_status()  # ステータスコードがエラーの場合例外を発生
except requests.exceptions.RequestException as e:
    print("error occured on requests.get(ranking_url, headers=HEADERS): ", e)

ranking_page = TRankingPage(html=tanomail_cashioning_package__response.text)
...
# CSVファイルに保存
csv_file = "cushioning_package_products(たのめーる).csv"
