...
from tanomail_2024_12.tanomail import BAKING_PAPER as TBAKING_PAPER
...
try:
    tanomail_baking_paper_response = requests.get(
        TBAKING_PAPER["url"],
        headers=HEADERS,
    )
    tanomail_baking_paper_response.encoding = tanomail_baking_paper_response.apparent_encoding
    tanomail_baking_paper_response.raise_for_status()  # ステータスコードがエラーの場合例外を発生
except requests.exceptions.RequestException as e:
    print("error occured on requests.get(ranking_url, headers=HEADERS): ", e)

ranking_page = TRankingPage(html=tanomail_baking_paper_response.text)
...
# CSVファイルに保存
csv_file = "baking_paper_products(たのめーる).csv"
