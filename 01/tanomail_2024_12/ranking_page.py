...
class RankingPage:
    ...
    def get_rankings(self) -> List[Ranking]:
        ...
        for i, item in enumerate(
            self.soup.select(".uk-flex.uk-gap-large.uk-width-1-1")
        ):
            if i >= 10:  # 10回で終了
                break
            rank = i + 1

            name_tag = item.select_one("h2.uk-h4 > a")
            name = name_tag.get_text(strip=True)
            variation_page_url = tanomail.SITE_BASE_URL + name_tag.get("href")
            ranking = Ranking(
...
if __name__ == "__main__":
    try:
        card_board_response = requests.get(
            "https://www.tanomail.com/ranking/tools/package/packing/cardboard/",
            headers=HEADERS,
        )
