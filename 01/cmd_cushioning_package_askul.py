...
from monotaro_2024_12.monotaro import CUSHIONING_PACKAGE as ACUSHIONING_PACKAGE
...
try:
    askul_cushioning_package_response = requests.get(
        ACUSHIONING_PACKAGE["url"],
        headers=HEADERS,
    )
    askul_cushioning_package_response.encoding = askul_cushioning_package_response.apparent_encoding
    askul_cushioning_package_response.raise_for_status()  # ステータスコードがエラーの場合例外を発生
except requests.exceptions.RequestException as e:
    print("error occured on requests.get(ranking_url, headers=HEADERS): ", e)

ranking_page = ARankingPage(html=askul_cushioning_package_response.text)
...
# CSVファイルに保存
csv_file = "cushioning_package_products(アスクル).csv"
