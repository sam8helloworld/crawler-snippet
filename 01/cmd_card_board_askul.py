...
from monotaro_2024_12.monotaro import CARDBOARD as ACARDBOARD
...
try:
    askul_card_board_response = requests.get(
        ACARDBOARD["url"],
        headers=HEADERS,
    )
    askul_card_board_response.encoding = askul_card_board_response.apparent_encoding
    askul_card_board_response.raise_for_status()  # ステータスコードがエラーの場合例外を発生
except requests.exceptions.RequestException as e:
    print("error occured on requests.get(ranking_url, headers=HEADERS): ", e)

ranking_page = ARankingPage(html=askul_card_board_response.text)
...
# CSVファイルに保存
csv_file = "card_board_products(アスクル).csv"
